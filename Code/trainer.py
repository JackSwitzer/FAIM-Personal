...
class LSTMTrainer:
    ...
    def _create_dataloader(self, data, seq_length, batch_size, num_workers=Config.NUM_WORKERS, shuffle=False):
        if data is None or data.empty:
            self.logger.warning("Attempted to create DataLoader with empty or None dataset.")
            return None

        dataset = SequenceDataset(data, seq_length, self.feature_cols, self.target_col)
        if len(dataset) == 0:
            self.logger.warning("SequenceDataset is empty. Check your data and sequence length.")
            return None
        ...
        data_loader_args = {
            ...
            'pin_memory': True,
            'shuffle': shuffle and not self.use_distributed  # Only shuffle if not using distributed training
        }
        ...
        if self.use_distributed:
            ...
            data_loader_args['shuffle'] = False  # When using a sampler, shuffle should be False
    ...
    def _evaluate(self, model, dataloader, criterion, return_predictions=False):
        if dataloader is None or len(dataloader) == 0:
            self.logger.warning("Evaluation DataLoader is None or empty.")
            return None, None, None, None if return_predictions else None
    ...
    def evaluate_test_set(self, model, test_data, hyperparams):
        ...
        test_loss, r2_score, predictions, targets = self._evaluate(model, test_loader, nn.MSELoss(), return_predictions=True)
        return test_loss, r2_score, predictions, targets
    ...
    def create_future_data_loader(self, future_data, seq_length, batch_size):
        ...
        future_dataset = SequenceDataset(
            self.data_processor.stock_data, seq_length, self.feature_cols, self.target_col
        )
        future_loader = DataLoader(
            future_dataset, batch_size=batch_size, num_workers=Config.NUM_WORKERS, shuffle=False
        )
    ...
    def predict_future_data(self, model, future_data_loader):
        ...
        predictions = []
        with torch.no_grad():
            for batch_X in future_data_loader:
                batch_X = batch_X.to(self.device, non_blocking=True)
                outputs = model(batch_X).squeeze(-1)
                predictions.extend(outputs.cpu().numpy())
        return predictions
